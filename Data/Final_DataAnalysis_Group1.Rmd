---
title: "Data Analysis"
author: "Sai Powar, Meera Ayyagari, and Ina Liao"
date: "2024-04-10"
output:
...
knitr:::opts_chunk$set(tidy.opts=list(width.cutoff=80), tidy=FALSE,fig.align = "center", fig.pos = "H")
```
...
ts_water_monthly<-ts(df_water_monthly[,2], frequency=12,start =c(2002,02,01), end=c(2021,10,01))
head(ts_water_monthly)
tail(ts_water_monthly)
#autoplot(msts_water)
...
autoplot(ts_testing2)+
  autolayer(NN_forecast_3,series="Neural Network c(6)")+
  autolayer(sarima_for, series = "SARIMA", PI=FALSE)+
...
autoplot(NN_for_all, PI=TRUE)
head(NN_for_all$mean)
tail(NN_for_all$mean)
...
Final_Forecast <- data.frame(NN_for_all)
Final_Forecast
write.csv(Final_Forecast, file="./Data/Final_Forecast.csv", row.names=F)
```
