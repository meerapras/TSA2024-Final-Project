...
#ts 
#decompose 
water_decompose<-decompose(msts_water_monthly)
...
#deseason series
ts_deseason_training<-ts(water_deseason,
                         frequency=12,
                         start=c(2001,03), end=c(2020,09))
ts_deseason_testing<-ts(water_deseason,
                        frequency=12,
                        start=c(2020,10), end=c(2021,10))
```
...
```{r ETS}
ETS_fit<-stlf(ts_training,h=12*3)
```

```{r ARIMA and Fourier terms}
#K=c(2)
#fit ARIMA with deseason time series 
ARIMA_Four_fit_1<-auto.arima(ts_deseason_training,
                             seasonal=FALSE, 
                             xreg=fourier(ts_training,K=c(2)))
#forecast with ARIMA_fit
ARIMA_Four_forecast_1<-forecast(ARIMA_Four_fit_1,
                                xreg=fourier(ts_training,K=c(2),h=12*3),
                                h=12*3) 
#K=c(4)
#fit ARIMA with deseason time series 
ARIMA_Four_fit_2<-auto.arima(ts_deseason_training,
                             seasonal=FALSE, 
                             xreg=fourier(ts_deseason_training,K=c(2)))
#forecast with ARIMA_fit
ARIMA_Four_forecast_2<-forecast(ARIMA_Four_fit_2,
                                xreg=fourier(ts_deseason_training,K=c(2),h=12*3),
                                h=12*3) 
...
ARIMA_Four_forecast_3<-forecast(ARIMA_Four_fit_3,
                                xreg=fourier(ts_training,K=c(6),h=12*3),
                                h=12*3) 

autoplot(ts_testing)+
  autolayer(ARIMA_Four_forecast_2)
```
...
#K=c(4)
#fit ARIMA with deseason time series 
ARIMA_Four_fit_XREG_1<-auto.arima(ts_deseason_training,
                                  seasonal=FALSE,
                                  #combine fourier and exogenous variable time series  
                                  xreg=as.matrix(cbind(fourier(ts_deseason_training,K=c(4)),
                                                       ts_temp_training)))  


#generate fourier terms
fourier_terms_1<-fourier(ts_deseason_training, K=c(4), h=12)
#generate forecast
...
#generate fourier terms
fourier_terms_2<-fourier(ts_training, K=c(6), h=12)
#generate forecast
...
#K=c(4)
NN_fit_1<-nnetar(ts_deseason_training,
                 p=1,
                 P=0,
                 xreg=fourier(ts_deseason_training,K=c(2)))
...
ETS_fit$mean<-ETS_fit$mean+water_decompose$seasonal
ARIMA_Four_forecast_1$mean<-ARIMA_Four_forecast_1$mean+water_decompose$seasonal
NN_forecast_1$mean<-NN_forecast_1$mean+water_decompose$seasonal
ARIMA_Four_forecast_XREG_1$mean<-ARIMA_Four_forecast_XREG_1$mean+water_decompose$seasonal
